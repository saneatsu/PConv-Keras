...
class DataGenerator(ImageDataGenerator):
    ...
    def has_not_255(self, masked):
        """
        チャンネルに255があるか判定
        1チャンネルでも255を持っていた場合Falseを返してやり直し
        """
        masked = np.uint8((masked[0,:,:,:] * 1.)*255)
        for i in range(masked.shape[0]): # 512
            for j in range(masked.shape[1]): # 512
                if masked[i,j,0] == 255 or masked[i,j,1] == 255 or masked[i,j,2] >= 255:
                    return False
    ...
    def has_many_mask(self, mask):
        """
        白地に黒色のマスクが一つも無い場合、Falseを返してクロップ処理やり直し
        """
    ...
    def flow_from_directory(self, directory_small, directory_medium, directory_large, *args, **kwargs):                          
        ...
        while True:
            ...
            if self.has_not_255(croped_ori):
                continue
